...
pnpm monorepo. The published package lives in `packages/oxc-config`; `playground/`
is a Next.js app that dogfoods the config. This file is the source of truth for
the package README — `packages/oxc-config/scripts/copy-readme.ts` copies it into
the package on `prepublishOnly` (the in-package copy is gitignored).
